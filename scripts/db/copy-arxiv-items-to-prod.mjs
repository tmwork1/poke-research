// 使い捨てスクリプト（2026-07-09 arXiv論文収集の初期投入用）。
//
// ローカルで収集・AIレビュー済みの items.kind='paper'（arXiv収集）を、AIレビュー結果ごと
// 本番Supabaseへそのままコピーする。本番でOpenAIレビューを再実行しない（課金の二重発生を
// 避ける）ことが目的。source は arXiv 1件のみ、tags/items は external_url・name の
// UNIQUE制約による自然キーで本番側にupsertし、本番採番のidをitem_tagsの関連付けに使う。
//
// 対象は items.kind='paper' のみ（既存の kind='article' には一切触れない）。
//
// Usage:
//   node scripts/db/copy-arxiv-items-to-prod.mjs --dry-run [--include-rejected]
//   node scripts/db/copy-arxiv-items-to-prod.mjs --apply [--include-rejected]
//
...
const ARXIV_ORIGIN_URL = 'https://arxiv.org/';
...
async function main() {
	...
	try {
		const sourceRes = await localClient.query('select id, name, type, origin_url, metadata from sources where origin_url = $1', [ARXIV_ORIGIN_URL]);
		if (sourceRes.rows.length === 0) {
			throw new Error('ローカルにarXivソース（sources.origin_url=https://arxiv.org/）が見つかりません。先に収集を実行してください。');
		}
		...
		const itemsRes = await localClient.query(
			`select id, external_url, kind, title, authors, summary, published_at, updated_at, metadata, version, body, ai_accepted, language
			 from items where kind = 'paper' ${acceptedFilter} order by id`,
		);
		...
		console.log(
			`[local] コピー対象 items.kind='paper': ${localItems.length} 件` +
		...
		console.log(
			`[prod見込み] 現状 kind='paper' 件数: ${prodExistingRes.rows.length} 件 / 新規insert見込み: ${toInsert.length} 件 / ` +
		...
		try {
			...
			for (const item of localItems) {
				...
				const itemUpsert = await prodClient.query(
					`insert into items (source_id, external_url, kind, title, authors, summary, published_at, updated_at, metadata, version, body, ai_accepted, language)
					 ...
					 on conflict (external_url) do update set
					   ...
					   ai_accepted = excluded.ai_accepted, language = excluded.language
					 returning id`,
					[
						prodSourceId,
						item.external_url,
