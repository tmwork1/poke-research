...
// hatena（はてなブックマーク検索RSS）は精度検証中のため既定の対象には含めない。使う場合は
// BACKFILL_TARGETS=qiita,zenn,note,blog,hatena のように明示的に指定すること。
...
const noteQueries = (process.env.BACKFILL_NOTE_QUERIES || 'ポケモン,ポケカ,ポケモンカード')
	...
	.map((q) => q.trim())
	.filter(Boolean);
const hatenaMaxCandidates = Number(process.env.BACKFILL_HATENA_MAX_CANDIDATES || '15');
...
async function main() {
	...
	if (targets.includes('hatena')) {
		add(summarize('hatena', await runImport('hatena', { maxCandidatesPerKeyword: hatenaMaxCandidates })));
	}
