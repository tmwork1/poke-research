...
import { buildSystemPrompt, computePromptVersion } from './ai-review-prompt.mjs';
...
export interface ImportArticleReview {
	...
	language: string;
	model: string;
	/** レビュー時点の system prompt のハッシュ（migrations/025、ai-review-prompt.mjs の computePromptVersion）。 */
...
function parseAiResponse(content: string): Omit<ImportArticleReview, 'model' | 'promptVersion'> {
...
export async function reviewImportArticle(input: ImportArticleReviewInput): Promise<ImportArticleReview> {
	...
	const promptVersion = await computePromptVersion(topic, input.kind);
	return {
		...review,
		model,
		promptVersion,
