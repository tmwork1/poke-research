...
// 判定でも無駄なくスキップされる。
// 2026-07-09 一時変更: 本番Cloudflare上での時間分割cron検証のため「時」を 0→8（UTC）へ一時的に
// ずらしている（wrangler.jsonc の同名エントリと完全一致させること）。検証後は 0 に戻す。
const DAILY_CRON = '0,5,10,15,20 8 * * *';
...
// 2026-07-09 一時変更: 上記 DAILY_CRON と同様、検証のため「時」を 0→8（UTC）へ一時的にずらしている。
// 検証後は 0 に戻す。
const DAILY_DIGEST_CRON = '40 8 * * *';
