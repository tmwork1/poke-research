...
--
-- ai_review_*（model/prompt_version/reason/confidence/reviewed_at）: 「今公開されている内容
...
-- 昇格させているのと同じ発想を、model/prompt_version/reason/confidence にも広げたもの。
--
-- ai_recheck_*（accepted/model/prompt_version/reason/confidence/rechecked_at）: 「直近の
...
-- items.metadata->'ai'->>'prompt_version' のjsonb抽出になり、構造が非対称でSQLで素直に比較
...
-- いる記事」を直接抽出できる:
--   SELECT * FROM items WHERE ai_accepted AND ai_review_prompt_version IS DISTINCT FROM ai_recheck_prompt_version;

ALTER TABLE items ADD COLUMN IF NOT EXISTS ai_review_model text;
ALTER TABLE items ADD COLUMN IF NOT EXISTS ai_review_prompt_version text;
...
-- 既存データの無料バックフィル: prompt_version/reviewed_at・rechecked_at はバージョン管理の
