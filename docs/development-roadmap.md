...
- [x] Qiita で確立した取り込み経路をもとに、Zenn / note（対応可能な範囲）へ収集対象を拡張する。
  ...
  - [x] はてなブックマークの検索RSS（`b.hatena.ne.jp/search/text?...&mode=rss`、認証不要）でキーワード横断検索し、発見した記事は個人ブログ収集と同じ汎用HTML抽出で取り込む（`src/lib/importers/hatena.ts`、`hatena-feed.ts`）。ブックマークされた全ウェブページを横断する検索のため一度は非公式利用を理由に見送ったが、再検討のうえ実装した（[2026-07-07](progress/2026-07-07.md)）。実測の結果、複数キーワードを与えても実質AND検索にならず無関係な話題性記事が混入することを確認しており、note収集と同種の「母集団の性質上の問題」と判断し、既存のAIレビューを安全網として運用しつつ候補数（既定15件/キーワード）を保守的に絞っている。
...
- [x] 検索とUIの両面で、一覧・トップ・詳細画面の情報設計を反復的に磨き込んだ（冗長な見出し・装飾・重複情報の削除、日付表示を元記事の公開日基準に変更など）。詳細は [2026-07-05](progress/2026-07-05.md) を参照。UI改善アイデアの棚卸しと実施記録は全項目完了のため撤去済み（[2026-07-05](progress/2026-07-05.md) に実施内容が残る）。
...
- 設計仕様書は実装完了に伴い撤去済み。実装内容・検証状況は [2026-07-05](progress/2026-07-05.md) を参照。
