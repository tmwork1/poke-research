...
`ai_last_review_*`列（AIレビューの直近判定を`ai_accepted`とは別に常に上書き記録する仕組み、[docs/progress/2026-07-10.md](../progress/2026-07-10.md)参照）を`migrations/025`として追加する際、2つの疑問が持ち上がった。

1. 列名が長すぎるのではないか（`ai_last_review_prompt_version`は29文字）。
...
| 025 | `ai_last_review_*`（6列） | 直近AIレビューの条件・結果（今回） |
...
- **接頭辞クラスタが「1:Nになりたがっているか」**（`link_*`や`ai_last_review_*`のような列の集まりが、単一の現在値ではなく複数の履歴/バリエーションを持ちたくなった瞬間）
...
実運用上の補足: `findExistingExternalUrls`が既存URLをOpenAI課金節約のため事前スキップする設計上、既存itemへの再レビュー自体が稀（`retag-existing-items.mjs`実行時、またはURL正規化ズレでの再突入時のみ）。履歴テーブルを作っても行がほとんど溜まらず、監査の実運用は結局「プロンプト更新後にretagを回して`ai_last_review_*`を見る」フローになる。
...
Fableも「`last_`は落とせる（スカラー列は定義上『最新値』しか持てないため、`link_checked_at`が`link_last_checked_at`でないのと同じ理屈）」と指摘した。`ai_last_review_*` → `ai_review_*`、`ai_last_reviewed_at` → `ai_reviewed_at`への改名は、`migrations/025`がまだ本番未適用の現時点が対応する唯一の無料タイミング。この改名を実施するかどうかはユーザーの最終判断待ち（本ドキュメント作成時点で未確定）。

## 参照

- [docs/progress/2026-07-10.md](../progress/2026-07-10.md) — `ai_last_review_*`列を追加するに至った経緯
- `migrations/016_add_link_status.sql` — 今回と構造的に同型の既存パターン
