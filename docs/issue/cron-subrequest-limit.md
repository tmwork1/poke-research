...
これで `wrangler.jsonc` の Cron Trigger 登録数はちょうどアカウント上限の5件
（週次レビュー・日次収集・リンク切れ検出・日次まとめ通知・ブログ）を使い切っている。
...
- zenn: 5+8×3+2 = **31**
- arxiv: 5+10×2+2 = **27**
- **feed: 21+10×4+2 = 63（上限50/呼び出しを超えうることが判明。要対応検討）**
...
- **blogは`'pokeapi'`検索に未収集候補が多数残っており、「新規0件」の定常状態を作れず固定コストを
  再測定できなかった。** 既存見積り（20〜23）のまま。
- **feedのワーストケース試算（63）が上限50/呼び出しを超える。** `FEED_MAX_NEW_PER_RUN`の
  引き下げ、または購読フィード数自体の増加抑制などの対応要否は未検討。
