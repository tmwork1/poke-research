...
## 2026-07-10: OpenAlexインポーター追加に伴う実測・cron統合

新規追加したOpenAlexインポーター（`src/lib/importers/openalex.ts`）を`DAILY_SLOT_JOBS`へ
組み込むにあたり、`DEFAULT_MAX_NEW_ITEMS_PER_RUN`（暫定値10、arXivからの類推）を
根拠付けるため、既存の計測手順（`DEBUG_SUBREQUEST_COUNT`・`scripts/eval/eval-subrequests.mjs`。
openalexルートを追加済み）で実測した。

### 実測結果

- 固定コスト（新規0件）: **5**（arXivと同値。同じ構造: 外部API取得1＋source upsert1＋
  top_tags取得1＋既存URL一括チェック1＋import_runs記録1）
- 新規1件あたり: **3**（arXivからの類推値「2」より1多い）

新規1件あたりが2ではなく3になった理由は、`ai_prompt_hashes`テーブル（migrations/026、PR #65）
追加に伴う`recordPromptHashIfNew`（`src/lib/importers/common.ts`）の存在。この関数はプロンプト
ハッシュ未記録時のみ`ai_prompt_hashes`へのupsertを1回発行する（Worker呼び出し内キャッシュあり）。
**この節の「2026-07-09: 実験によるsubrequest消費数の確定」の表にあるqiita/zenn/arxiv/feedの
値は、いずれもai_prompt_hashes追加（PR #65、2026-07-09時点でこの実験より後にマージ）より前の
実測・類推のため、実際には全ルートで同様に+1されている可能性が高い。** 再測定・既定値の
見直しは本対応のスコープ外とし、影響有無の確認は別途持ち越しとする。

### 既定値の決定

新規N件が全件新規だった場合のワーストケース = 固定コスト5 + N×3 + タグ同期バッチ概算2。
qiita(33)/zenn(31)/arxiv(27)が採用している安全マージン（上限50に対し15〜20程度の余裕）に
合わせ、N=12（ワーストケース43）を採用した（旧値10から引き上げ。`openalex.ts`の
`DEFAULT_MAX_NEW_ITEMS_PER_RUN`）。N=13以上（46以上）はfeed/blogと同程度まで余裕が
縮むため、新規にcron投入するジョブの初期値としては見送った。

### cron統合

`worker.ts`の`DAILY_SLOT_JOBS`に分30のスロットとして追加した（`wrangler.jsonc`の
`DAILY_CRON`文字列に`30`を追加、`"0,5,10,15,20,25,30,59 15 * * *"`）。新規のCron Trigger
登録は不要（既存の日次収集用エントリに分を1つ足しただけ）。実行順序はarXiv（分15）・
リンク切れ検出（分25）の後、日次まとめ通知（分59）の前に配置し、OpenAlexがarXiv由来と
判定した論文（arXivと同じ正規化URLを使う、`openalex-parse.ts`の`selectExternalUrl`）を
その回のarXiv収集結果に対しても無駄なくスキップできるようにした。
