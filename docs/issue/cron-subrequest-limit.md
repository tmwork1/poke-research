...
- **発覚日**: 2026-07-09
- **状態**: 差分検知・バッチ化を実施済み（ブランチ`fix/cron-subrequest-diff-detection`）。
  実験の結果それだけでは不十分と判明し、候補5のうちリンク切れ検出・ブログ（Brave Search）の
  専用cron分離（バッチサイズ自動算出・検索キーワードの巡回実行）、および全6収集ジョブへの
  新着処理件数上限（急増日対策）を実施済み。日次収集バンドル（フィード/Qiita/Zenn/はてな/arXivの
  5ジョブ）そのものをさらに分割する対応は未実施で引き続き検討課題。
