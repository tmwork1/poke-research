...
`merge-item` は `detect-duplicate-items` の出力で重複itemが見えたとき、著者・内容を確認して同一記事と判断できたペアにのみ使う（タイトルが似ているだけで内容が別の記事は統合対象外）。同様に `merge-source` は `detect-duplicate-sources` の出力で重複sourceが見えたときに使う。統合しないと判断した組は `dismiss-duplicate` で除外登録しておく（登録しないと週次DBレビューの通知に毎週同じ組が再掲される）。`merge-tag`（表記ゆれの統合）・`rename-tag`（冗長・冗長な接頭辞の短縮など単純リネーム）・`delete-tag`（検索価値の低い不適切タグの削除）は上記2つとは別に、`eval:all` の `eval:tags`（Claude Codeがこの出力を読み、リネーム/統合/削除を直接判断する。以前はこの判断を`optimize-tags.mjs`がOpenAIへ委ねていたが、`eval:tags`だけで判定材料が揃うため廃止した）出力でノイズタグ・表記ゆれ・短縮の余地が見えたときに使い分ける。`eval-annotations`・`eval-broken-links` は上記2トリガーとは独立に、annotations件数が増えてきたときや月次の目視確認のタイミングで都度実行する。

| スクリプト | コマンド | 用途 | 課金 |
|---|---|---|---|
| `scripts/db/detect-duplicate-items.mjs` | `npm run db:detect-duplicate-items` | 同一記事のクロスポスト候補を、URL正規化一致 or タイトル類似度で検出し一覧表示する。読み取り専用（DBに書き込まない）。「検出→確認→削除」を自動で一本化する対話的ヘルパーは、判断そのものを人手で行う必要があるためユーザー判断により不要と確定し、実装しない。統合は`merge-item.mjs`で行い、統合しないと判断した組は`dismiss-duplicate.mjs`で除外登録する（既定では除外済みの組を出力せず、`--include-dismissed`で全件表示できる）。`npm run eval:all` の既定ステップにも含まれる。 | なし |
...
| `scripts/db/dismiss-duplicate.mjs <item\|source> <id1> <id2> [--note=] [--undismiss] [--dry-run]` | `node --env-file=.env.production scripts/db/dismiss-duplicate.mjs item 3 1460 --note="別著者の別記事"` | 重複候補のうち「統合しない（別記事・別ソース）」と判断した組を`duplicate_review_dismissals`（migrations/030）に登録し、週次DBレビューのDiscord通知と`detect-duplicate-*.mjs`の出力から除外する。除外しないと同じ組が毎週再掲され、同じ判断をやり直すことになる。idは昇順に正規化して保存するため引数の順序は任意。`--undismiss`で解除、`--list`で一覧、`--dry-run`で書き込み内容の表示のみ。本番実行前にユーザー確認必須。 | なし |
| `scripts/db/detect-duplicate-sources.mjs` | `npm run db:detect-duplicate-sources` | sources専用の重複検出。`origin_url`の正規化一致・`name`の類似度で候補を出す（`detect-duplicate-items.mjs`の正規化ロジックを流用）。読み取り専用（DBに書き込まない）。統合は`merge-source.mjs`で行い、統合しないと判断した組は`dismiss-duplicate.mjs`で除外登録する（`--include-dismissed`で除外済みも表示）。 | なし |
...
| `scripts/db/backfill-item-bodies.mjs` | `node --env-file=.env.production scripts/db/backfill-item-bodies.mjs --route=qiita-importer --dry-run` | `items.body`がNULLまたは100字未満の行を対象に、Qiita / Zennの公開APIから本文を取り直し、各インポーターと同じ上限4000文字で保存する。更新は`body`のみで、AI再判定は行わない。`SUPABASE_URL`/`SUPABASE_SECRET_KEY`必須、`QIITA_TOKEN`は任意。`--route=`は`collection_route`で絞り込み、`--id=`は単件、`--limit=`は対象行数（対象外routeを含む）、`--interval=`は取得間隔（既定200ms）を指定する。その他のrouteはスキップし、件別の失敗後も続行して更新/失敗/スキップ件数を表示する。`--dry-run`はDB読み取りと対象表示のみで、本文API取得・DB更新は行わない。本番実行前にユーザー確認必須。 | なし |
...
| `scripts/db/apply-item-review.mjs (--id= --accepted= --language= --reason= [--confidence=] [--summary= --tags=] \| --from=<path>) [--model=] [--publish] [--dry-run]` | `node scripts/db/apply-item-review.mjs --id=123 --accepted=true --language=ja --reason="..." --summary="..." --tags="a,b"` | `list-retag-targets.mjs`で取得した1アイテムの判定結果を書き込む。`items.language`と`items.ai_recheck_*`（migrations/025）は`accepted`に関わらず常に更新する（既定では`ai_accepted`自体は書き換えない）。棄却済み記事を再採用して公開する場合は`--publish`で採用行の`ai_accepted=true`も更新する。非公開化は人手確認を要するため`--accepted=false --publish`はエラー。一括では採用行だけを公開する。`items.ai_review_*`（migrations/025）はsummary/タグと同時に採用時のみ更新する。不採用判定になった場合はsummary/タグを自動削除せず警告のみ。`ai_review_prompt_hash`/`ai_recheck_prompt_hash`には引き続き`computePromptHash(topic)`を書き込み、どの採否基準バージョンで判定したかを追跡できる。`--from=<path>`で1行1件のJSONL（`id`整数・`accepted`真偽値・`language`/`reason`必須・採用時`summary`必須・`confidence`は0〜1か未指定・`tags`は文字列配列、棄却行は`summary=""`/`tags=[]`）を順に適用する。全行を事前検証し、不正な行があれば書き込みなしで終了する。`--id`との併用は不可。`--dry-run`/`--publish`と併用でき、例: `node scripts/db/apply-item-review.mjs --from=reviews.jsonl --publish --dry-run`。dry-runはDB接続不要で採用・棄却・公開対象の件数と各行の内容を表示する。各行の成否と最後に成功/失敗件数を出力し、適用に失敗しても後続を継続して終了コードを非0にする。`--model`は判定者識別子（既定`claude-code`、サブエージェント使用時は実際のモデル名を渡す）。本番実行前にユーザー確認必須。 | なし |
| `scripts/db/unpublish-items.mjs (--ids=1,2,3 または --from=<path>) --reason="..." [--model=] [--dry-run]` | `node scripts/db/unpublish-items.mjs --ids=1,2,3 --reason="リンク先が404" --dry-run` | 人手確認済みの記事を非公開化する。`apply-item-review.mjs` が自動では行わない `ai_accepted=false` と `ai_recheck_*` / `ai_rechecked_at` を更新し、`computePromptHash(topic)` を記録する。summary・タグは再公開に備えて保持する。`--from` は1行1IDのテキストまたは `{"id":123,"reason":"個別理由"}` のJSONL。理由は必須で、個別理由を共通の `--reason` より優先する。全入力を事前検証し、重複IDはエラー。`--model` は判定者識別子（既定 `human-review`）。`SUPABASE_URL` / `SUPABASE_SECRET_KEY` が必要。dry-runもDBを読み取り、ID・タイトル・現在の公開状態だけを表示する。実行時は1件ずつ成否と最後に非公開化/失敗件数を出し、失敗後も続行して終了コードを非0にする。本番実行前にユーザー確認必須。 | なし |
